...
def emit(buffer_r, buffer_s, delta_time=sys.maxsize, leading=None):
    ...
    if r_2 is not None:
        while s_0 is not None and s_1.get("record").get_time() < r_2.get("record").get_time():
            if r_1.get("record").get_time() < s_0.get("record").get_time():
                # print("in part 1: ", end="")
                join(r_1, s_1, delta_time, leading)
    ...
    while s_1 is not None and s_1.get("record").get_time() < r_1.get("record").get_time():
        if r_2 is not None and r_2.get("record").get_time() < s_1.get("record").get_time():
            join(r_2, s_1, delta_time, leading)
    ...
    buffer_r = strip_buffers(buffer_r, buffer_s, delta_time)
    buffer_s = strip_buffers(buffer_s, buffer_r, delta_time)

    return buffer_r, buffer_s


def strip_buffers(br, bs, delta_time):
    # if len(buffer_s) == 0:
    #     return buffer_r
    ...
    while r_2 is not None and r_1.get("record").get_time() < r_2.get("record").get_time() <= s_1.get("record").get_time():
        ...
        if not r_1.get("was_older"):
            ...
            while s_0 is not None and s_0.get("record").get_time() <= r_1.get("record").get_time():
                s_idx += 1
                s_0 = bs[s_idx]
            join(r_1, s_0, delta_time, leading)
...
def join(r, s, delta_time, leading=None):
    # check the delta time constraint, don't join if not met
    if abs(r.get('record').get_time() - s.get('record').get_time()) <= delta_time:
        # print the leading quantity at first
        if s.get('record').get_quantity() == leading:
    ...
    # else:
    #     print("  time constraint not met in join")


if __name__ == "__main__":
    ts = time.time()

    # time delta parameter for time-series join
    delta_time = 2
    ...
    for i in range(N):
        ...
        if ingestionOrder[i] == "r":
            ...
            n_r += 1
            events_r = events_r[1:]
            buffer_r, buffer_s = emit(buffer_r, buffer_s, delta_time=delta_time, leading=leading)
            ...
            n_s += 1
            events_s = events_s[1:]
            buffer_s, buffer_r = emit(buffer_s, buffer_r, delta_time=delta_time, leading=leading)
