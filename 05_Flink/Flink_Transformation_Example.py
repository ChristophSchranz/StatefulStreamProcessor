...
from pyflink.table.descriptors import Kafka, Json, OldCsv
sourcetable = table_env.connect(Kafka.properties({'update-mode':'append', 'connector.topic':'machine.data','connector.properties.zookeeper.connect':'localhost:2181','connector.properties.bootstrap.servers':'localhost:9092'})).with_format(Json().json_schema("{type:'object',properties:{thing: {type: 'string'},quantity:{type:'string'},phenomenonTime:{type:'integer'},result:{type:'number'}}}")).with_schema(Schema().field("thing", "VARCHAR").field("quantity","VARCHAR").field("phenomenonTime","INT").field("result","DOUBLE")).register_table_source("Source")
